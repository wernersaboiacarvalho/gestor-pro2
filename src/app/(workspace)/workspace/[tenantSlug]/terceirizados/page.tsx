...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return { title: `Terceirizados - ${tenantSlug}` }
}
...
export default async function PartnersPage({ params, searchParams }: Props) {
  ...
  return (
    <div>
      ...
      <div className="mb-6">
        <SearchInput placeholder="Buscar por nome, serviço ou contato..." basePath={`/workspace/${tenantSlug}/terceirizados`} defaultValue={q} />
      </div>
      ...
      {partners.length === 0 ? (
        ...
        <>
          ...
          <Pagination currentPage={page} totalPages={totalPages} basePath={`/workspace/${tenantSlug}/terceirizados`} searchParams={{ q }} />
        </>
      )}
    </div>
  )
